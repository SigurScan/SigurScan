...
# #81: enforce by default in production. The static APK key alone is
# extractable; enforce requires a valid Play Integrity token on guarded POST
# routes. Override with PLAY_INTEGRITY_MODE=monitor for test/staging deploys.
PLAY_INTEGRITY_MODE="${PLAY_INTEGRITY_MODE:-enforce}"
