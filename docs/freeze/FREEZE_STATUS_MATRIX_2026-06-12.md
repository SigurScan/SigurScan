...
| 3 | Supabase | Green for runtime DB path, Partial for resilience | Remote migration list matches local migrations. Required tables exist, RLS is enabled, `anon`/`authenticated` have no direct grants on runtime tables, preview bucket `previews` is private PNG-only with 5MB limit, visual-only constraints exist, and a live scan wrote `orch_1781289050_c007425e` to `scan_jobs`. Runtime access is Data API/PostgREST, not long-lived direct DB connections; a 20-concurrent admin telemetry pressure check returned `20/20 HTTP 200` on both official and raw Run paths. Supabase CLI proof shows `pitr_enabled=false`, `walg_enabled=true`, `backups=[]`; database size is currently `26 MB`. Local schema/data emergency dumps exist outside the repo. Automated daily logical backup tooling now exists through GitHub Actions artifact retention, but this remains Partial until `SUPABASE_DB_URL` is configured as a GitHub secret and the first workflow artifact succeeds. |
...
3. Configure GitHub secret `SUPABASE_DB_URL`, run the `Supabase logical backup` workflow manually once, and attach the successful artifact proof. PITR remains optional paid resilience later.
