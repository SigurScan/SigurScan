...
async def _finalize_orchestrated_job_if_ready(job: Dict[str, Any], request: Request) -> Dict[str, Any]:
    ...
    if existing_result and existing_result.get("is_final", True) is not False:
        if not main._urlscan_enhancement_done(job) and not main._urlscan_finished_with_risk(job):
            return job
    if not main._orchestrated_can_finalize_result(job, pillars):
        return job
    ...
    evidence = analysis.get("evidence", {}) if isinstance(analysis.get("evidence"), dict) else {}
    gate = evidence.get("verdict_gate") if isinstance(evidence.get("verdict_gate"), dict) else {}
    if str(gate.get("label") or "").upper() == "UNVERIFIED" and not main._orchestrated_result_is_final(job, analysis):
        if existing_result and existing_result.get("is_final", True) is not False:
            main._emit_orchestrated_telemetry("orchestrated_verdict_pending_preserved_final", job)
            return job
        job.pop("result", None)
        job.pop("result_fingerprint", None)
        main._emit_orchestrated_telemetry("orchestrated_verdict_pending", job)
        return job
    fingerprint = main._orchestrated_result_fingerprint(job, analysis, pillars, resolved_urls)
    ...
    response_payload["is_final"] = (
        main._orchestrated_result_is_final(job, analysis)
        and (
...
async def _submit_orchestrated_urlscan_preview_once(job: Dict[str, Any], request: Request) -> Dict[str, Any]:
    ...
    if primary_final_url and primary_url_privacy.get("preview_allowed") is False:
        ...
        main._set_orchestrated_stage(job, "urlscan_submitted")
        job = main._persist_orchestrated_job(job)
        main._emit_orchestrated_telemetry("orchestrated_urlscan_privacy_skipped", job)
        return job
    ...
    if primary_final_url and urlscan_status in {"queued", "", "skipped"}:
        ...
        if cached_preview:
            ...
            if cached_fast_preview:
                ...
                main._emit_orchestrated_telemetry("orchestrated_fast_preview_cache_hit", job)
            main._set_orchestrated_stage(job, "urlscan_submitted")
            job = main._persist_orchestrated_job(job)
            main._emit_orchestrated_telemetry("orchestrated_urlscan_preview_cache_hit", job)
            return job
        ...
        main._set_orchestrated_stage(job, "urlscan_submitting")
        job = main._persist_orchestrated_job(job)
        urlscan_state = job.get("urlscan") if isinstance(job.get("urlscan"), dict) else {}
        if urlscan_state.get("submit_owner") != submit_owner or urlscan_state.get("uuid"):
            main._increment_orchestrated_metric(job, "urlscan_reservation_guard_hits")
            main._emit_orchestrated_telemetry("orchestrated_urlscan_reservation_guard", job)
            return job
        ...
        started_at = time.perf_counter()
        try:
            submitted_urlscan = await main._submit_orchestrated_urlscan(str(primary_final_url), urlscan_payload, request)
        finally:
            main._record_orchestrated_component_duration(job, "urlscan.submit", started_at)
    ...
    main._set_orchestrated_stage(job, "urlscan_submitted")
    job = main._persist_orchestrated_job(job)
    main._emit_orchestrated_telemetry("orchestrated_urlscan_submitted", job)
    return job


async def _create_orchestrated_job(payload: OrchestratedScanRequest) -> Dict[str, Any]:
    context = main._build_orchestrated_text_context(payload)
    ...
    job = main._persist_orchestrated_job(job)
    main._emit_orchestrated_telemetry("orchestrated_created", job)
    return job
...
async def _run_orchestrated_fast_lane(job: Dict[str, Any], request: Request) -> Dict[str, Any]:
    ...
    resolved_urls = main._timed_orchestrated_component(
        job,
    ...
    main._set_orchestrated_stage(job, "resolved")
    main._emit_orchestrated_telemetry("orchestrated_stage_resolved", job, fast_lane=True)
    ...
    threat_intel = main._timed_orchestrated_component(
        job,
    ...
    summary = main._timed_orchestrated_component(
        job,
    ...
    if main._has_bad_provider_verdict(summary):
        analysis = main._timed_orchestrated_component(
            job,
        ...
        main._timed_orchestrated_component(
            job,
        ...
        claim_required = main._timed_orchestrated_component(
            job,
    ...
    primary_entry = main._timed_orchestrated_component(
        job,
    ...
    main._set_orchestrated_stage(job, next_stage)
    job = main._timed_orchestrated_component(
        job,
        f"fast_lane.persist_{next_stage}",
        lambda: main._persist_orchestrated_job(job),
    )
    main._emit_orchestrated_telemetry(
    ...
    if main.ORCHESTRATED_EARLY_VERDICT and next_stage == "semantic_ready":
        ...
        job = await main._finalize_orchestrated_job_if_ready(job, request)
    return job
...
async def _run_orchestrated_invoice_fast_lane(job: Dict[str, Any], request: Request) -> Dict[str, Any]:
    ...
    if urls:
        ...
        external_intel_summary = main._timed_orchestrated_component(
            job,
            "invoice_fast_lane.reputation_summary",
            lambda: main._external_intel_summary_from_threat_intel(threat_intel),
        )
        job["threat_intel"] = threat_intel
    main._set_orchestrated_stage(job, "invoice_parse")
    try:
        result = await main._timed_orchestrated_component(
            job,
        ...
        result = None
        main._emit_orchestrated_telemetry("orchestrated_invoice_error", job, error=str(exc))
    ...
    main._set_orchestrated_stage(job, "analysis_ready")
    job = main._persist_orchestrated_job(job)
    main._emit_orchestrated_telemetry("orchestrated_stage_analysis_ready", job, invoice_fast_lane=True)
    return job
...
async def _run_orchestrated_offer_fast_lane(job: Dict[str, Any], request: Request) -> Dict[str, Any]:
    ...
    main._set_orchestrated_stage(job, "offer_parse")
    try:
        result = await main._timed_orchestrated_component(
            job,
        ...
        result = None
        main._emit_orchestrated_telemetry("orchestrated_offer_error", job, error=str(exc))
    ...
    main._set_orchestrated_stage(job, "analysis_ready")
    job = main._persist_orchestrated_job(job)
    main._emit_orchestrated_telemetry("orchestrated_stage_analysis_ready", job, offer_fast_lane=True)
    return job
...
def _mark_orchestrated_job_exception(job: Dict[str, Any], exc: Exception) -> Dict[str, Any]:
    ...
    main._set_orchestrated_stage(job, "done")
    ...
    job["result"] = response_payload
    job["result_fingerprint"] = main._orchestrated_result_fingerprint(
        job,
        analysis,
        main._build_orchestrated_pillars(job),
        job.get("resolved_urls") if isinstance(job.get("resolved_urls"), list) else [],
    )
    job["status"] = "complete"
    main._emit_orchestrated_telemetry("orchestrated_unhandled_exception_finalized", job, error_type=error_type)
    if response_payload.get("is_final"):
        try:
            ...
            pass
    return main._persist_orchestrated_job(job)


async def _refresh_orchestrated_job(job: Dict[str, Any], request: Request) -> Dict[str, Any]:
    try:
        return await main._refresh_orchestrated_job_impl(job, request)
    except Exception as exc:
        return main._mark_orchestrated_job_exception(job, exc)


async def _refresh_orchestrated_job_impl(job: Dict[str, Any], request: Request) -> Dict[str, Any]:
    main._increment_orchestrated_metric(job, "poll_count")
    stage = str(job.get("pipeline_stage") or "queued").strip().lower()
    main._emit_orchestrated_telemetry("orchestrated_poll", job, stage=stage)
    existing_result = job.get("result") if isinstance(job.get("result"), dict) else None
    ...
    if (
        ...
        return await main._finalize_orchestrated_job_if_ready(job, request)
    if not existing_result and main._orchestrated_required_pillars_timed_out(job):
    ...
    if stage == "queued":
        ...
        if job_input_type == "invoice":
            return await main._run_orchestrated_invoice_fast_lane(job, request)
        if job_input_type == "offer":
            return await main._run_orchestrated_offer_fast_lane(job, request)
        return await main._run_orchestrated_fast_lane(job, request)
    ...
    if stage == "resolved":
        ...
        if main._has_bad_provider_verdict(summary):
            ...
            main._set_orchestrated_stage(job, "analysis_ready")
            job = main._persist_orchestrated_job(job)
            main._emit_orchestrated_telemetry("orchestrated_stage_analysis_ready", job, decisive_provider=True)
            return await main._finalize_orchestrated_job_if_ready(job, request)
        ...
        job["claim_verifier_required"] = False
        main._set_orchestrated_stage(job, "urlhaus_ready")
        job = main._persist_orchestrated_job(job)
        main._emit_orchestrated_telemetry("orchestrated_stage_urlhaus_ready", job)
        return job
    ...
    if stage == "urlhaus_ready":
        ...
        job["analysis"] = analysis
        main._set_orchestrated_stage(job, "reputation_ready")
        job = main._persist_orchestrated_job(job)
        main._emit_orchestrated_telemetry("orchestrated_stage_reputation_ready", job)
        return job
    ...
    if stage == "reputation_ready":
        ...
        job["claim_verifier_required"] = claim_required
        main._apply_primary_resolved_url(job, primary_entry)
        main._set_orchestrated_stage(job, "semantic_ready")
        job = main._persist_orchestrated_job(job)
        main._emit_orchestrated_telemetry("orchestrated_stage_semantic_ready", job, claim_required=claim_required)
        return job
    ...
    if stage == "claim_ready":
        ...
        main._set_orchestrated_stage(job, "analysis_ready")
        job = main._persist_orchestrated_job(job)
        main._emit_orchestrated_telemetry("orchestrated_stage_analysis_ready", job, claim_required=claim_required)
        return await main._finalize_orchestrated_job_if_ready(job, request)

    if stage == "analysis_ready":
        job = await main._submit_orchestrated_urlscan_preview_once(job, request)
        return await main._finalize_orchestrated_job_if_ready(job, request)
    ...
    if stage == "urlscan_submitting":
        ...
        if (
            ...
            main._increment_orchestrated_metric(job, "urlscan_reclaim_count")
            main._set_orchestrated_stage(job, "analysis_ready")
            job = main._persist_orchestrated_job(job)
            main._emit_orchestrated_telemetry("orchestrated_urlscan_reclaimed", job, submit_age_seconds=submit_age)
            return job
        return await main._finalize_orchestrated_job_if_ready(job, request)
        ...
        try:
            if urlscan_status == "finished" and not urlscan_state.get("screenshot_ready"):
                ...
                try:
                    ...
                    main._record_orchestrated_component_duration(job, "urlscan.screenshot_probe", started_at)
                ...
                if screenshot_ready:
                    ...
                    main._increment_orchestrated_metric(job, "urlscan_timeout_count")
                ...
                try:
                    ...
                    main._record_orchestrated_component_duration(job, "urlscan.result_poll", started_at)
        except HTTPException as exc:
    ...
    job = main._persist_orchestrated_job(job)
    main._emit_orchestrated_telemetry("orchestrated_urlscan_polled", job)
    return await main._finalize_orchestrated_job_if_ready(job, request)
...
async def _start_orchestrated_from_extraction(
    ...
    *,
    ...
    job = await main._create_orchestrated_job(
    ...
    response = main._orchestrated_status_payload(job)
