...
class CfxStore:
    def __init__(self, *, load_remote: bool = True):
    ...
    def _load_remote(self) -> None:
        try:
            ...
            rows = supabase_store.load_campaign_fingerprints()
        except Exception:
            rows = []
        for row in rows:
            try:
                fp = CampaignFingerprint.from_dict(row)
            except Exception:
                continue
    ...
    def put(self, fp: CampaignFingerprint) -> None:
        ...
        try:
            from services import supabase_store
            supabase_store.save_campaign_fingerprint(fp.to_dict())
        except Exception:
            pass
    ...
    def delete(self, fp_id: str) -> None:
        ...
        try:
            from services import supabase_store
            supabase_store.delete_campaign_fingerprint(fp_id)
        except Exception:
            pass
