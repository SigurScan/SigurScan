...
class TestCfxStore:
    def test_store_loads_persisted_fingerprint(self, monkeypatch):
        monkeypatch.setattr(
            supabase_store,
            "load_campaign_fingerprints",
            lambda: [
                {
                    "fingerprint_id": "cf_remote_001",
                    "locale": "ro-RO",
                    "channel_class": "sms",
                    "arc_family": "CONV_COURIER_TAX_CARD",
                    "ask_sequence_sig": "urgency",
                    "cta_pattern_sig": "card_details",
                    "identity_claim_sig": "courier",
                    "payment_rail_sig": "card_form",
                    "sensitive_request_sig": ["card"],
                    "text_skeleton_hash": "a" * 16,
                    "url_shape_sig": "brand-lookalike-domain",
                    "no_raw_iocs": True,
                    "created_at": 1.0,
                }
            ],
        ...
        assert store.get("cf_remote_001") is not None
        assert store.get("cf_remote_001").arc_family == "CONV_COURIER_TAX_CARD"

    def test_store_persists_and_deletes_fingerprint(self, monkeypatch):
        captured = {}
        deleted = []
        monkeypatch.setattr(supabase_store, "load_campaign_fingerprints", lambda: [])
        monkeypatch.setattr(
            supabase_store,
            "save_campaign_fingerprint",
            lambda entry: captured.setdefault("entry", entry),
        )
        monkeypatch.setattr(supabase_store, "delete_campaign_fingerprint", lambda fp_id: deleted.append(fp_id))
        store = CfxStore(load_remote=False)
        fp = extract_fingerprint("taxa colet card urgent", channel="sms", claimed_identity="FAN Courier")
        fp.fingerprint_id = "cf_persist_001"

        store.put(fp)
        store.delete("cf_persist_001")

        assert captured["entry"]["fingerprint_id"] == "cf_persist_001"
        assert deleted == ["cf_persist_001"]
        assert store.get("cf_persist_001") is None
