-- §9 Persistence tables for truth_registry, campaign_fingerprints,
--    circle_links, guardian_*, call_radar_hot_cache.

-- Brand Truth Registry manifests (mirrors brand_truth_registry_v1.json)
...
-- Campaign fingerprints (mirrors cfx_engine.CampaignFingerprint)
...
-- Guardian audit logs (moderation, approvals)
...
-- Guardian dangerous intel approvals
...
create index if not exists brand_truth_manifest_confidence_idx on public.brand_truth_manifest (confidence);
create index if not exists brand_truth_manifest_review_status_idx on public.brand_truth_manifest (review_status);
create index if not exists campaign_fingerprint_arc_family_idx on public.campaign_fingerprint (arc_family);
create index if not exists campaign_fingerprint_locale_idx on public.campaign_fingerprint (locale);
create index if not exists circle_link_source_idx on public.circle_link (source_intel_id);
create index if not exists circle_link_target_idx on public.circle_link (target_intel_id);
create index if not exists circle_link_type_idx on public.circle_link (link_type);
create index if not exists guardian_audit_log_created_at_idx on public.guardian_audit_log (created_at desc);
create index if not exists guardian_audit_log_target_idx on public.guardian_audit_log (target_type, target_id);
create index if not exists guardian_dangerous_approval_intel_id_idx on public.guardian_dangerous_approval (intel_id);
create index if not exists call_radar_hot_cache_target_hash_idx on public.call_radar_hot_cache (target_hash);
create index if not exists call_radar_hot_cache_expires_at_idx on public.call_radar_hot_cache (expires_at);
create index if not exists call_radar_hot_cache_risk_level_idx on public.call_radar_hot_cache (risk_level);
